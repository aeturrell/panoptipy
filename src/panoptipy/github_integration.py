...
class GithubScanResult:
    ...
    def successful_repos(self) -> List[str]:
        ...
        return [
            name for name, result in self.repo_results.items() if result.error is None
        ]
...
class GithubScanner:
    ...
    def _get_team_id(self, org_name: str, team_name: str) -> str:
        ...
        variables = {"org": org_name, "team": team_slug}
        ...
        try:
            ...
            if not data["organization"].get("team"):
                raise ValueError(
                    f"Team '{team_name}' not found in organization '{org_name}'"
                )
    ...
    def _get_team_repos(
        self, team_id: str, skip_forks: bool = True, skip_archived: bool = True
    ) -> List[Dict[str, Any]]:
        ...
        while has_next_page:
            variables = {"teamId": team_id, "cursor": cursor}
            ...
            for repo in repos_data["nodes"]:
                # Apply filters
                if (skip_forks and repo["isFork"]) or (
                    skip_archived and repo["isArchived"]
                ):
                    continue
    ...
    def _clone_repo(
        self, repo_info: Dict[str, Any], target_dir: Union[str, Path]
    ) -> Path:
